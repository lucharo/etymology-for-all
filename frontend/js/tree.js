 ...
 * @param {Function} onNodeClick - Callback when a node is clicked (receives node data)
 * @returns {string} HTML string of the rendered tree
 */
export function renderTreeHTML(tree, onNodeClick) {
 ...
 */
export function getTreeDepth(tree, currentDepth = 0) {
    if (!tree || tree.children.length === 0) return currentDepth;
